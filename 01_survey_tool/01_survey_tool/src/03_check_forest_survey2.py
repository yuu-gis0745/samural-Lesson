...
# 空欄、Noneを判定する
def is_blank(value:str)-> str:
    """
    空欄判定
    None、空文字、スペースのみを空欄とみなす
    """
    # None、""、" "	、"　"、" "をNoneとするためstripでスペース等を削除する
...
def check_tree_rows(ws, sheet_name, tree_information: dict, start_row: int, end_row: int, tree_rules: list) -> list:
    """
    毎木調査欄を、check_rules_forest_survey.csv のルールでチェックする関数
    """
    ...
    for row in range(start_row, end_row + 1):
        ...
        # 完全な空行はスキップ
        if (
            is_blank(row_values.get("樹種"))
            and is_blank(row_values.get("樹高(m)"))
            and is_blank(row_values.get("枝下高(m)"))
            and is_blank(row_values.get("胸高直径(cm)"))
            and is_blank(row_values.get("異常区分"))
            and is_blank(row_values.get("被害区分"))
            and is_blank(row_values.get("備考"))):
            continue
